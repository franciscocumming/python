...
from autosub import Translator
...
class Ctr_Autosub():
    ...
    def generate_subtitles(# pylint: disable=too-many-locals,too-many-arguments
        ...
        dst_language = src_language_translate
        google_translate_api_key = ApiKey
        translator = Translator(dst_language, google_translate_api_key ,
                                            dst=src_language_translate,
                                            src=src_language)
        ...
        transcripts = []
        transcripts_translate = []
        if regions:
            try:
                if Ctr_Autosub.cancel:
                    return -1

                str_task_1 = "Paso 1 de 3: Extrayendo audio de video "
                ...
                str_task_2 = "Paso 2 de 3: Generando Transcripción "
                Ctr_Autosub.pool = multiprocessing.Pool(concurrency)
                ...
                if src_language_translate !=" ":
                    
                    str_task_3 = "Paso 3 de 3: Traduciendo  "
                    Ctr_Autosub.pool = multiprocessing.Pool(concurrency)
                    for i, transcript in enumerate(Ctr_Autosub.pool.imap( translator, transcripts)):
                        Ctr_Autosub.step = 3

                        transcripts_translate.append(transcript)
                        progress_percent = MyUtil.percentage(i, len_regions)
                        Ctr_Autosub.output_progress(listener_progress, str_task_3, progress_percent)
                    ...
                    if Ctr_Autosub.cancel:
                        ...
                        Ctr_Autosub.pool.close()
                        Ctr_Autosub.pool.join()
                    transcripts= transcripts_translate
                else:
                    transcripts_translate=transcripts
        ...
        timed_subtitles = [(r, t) for r, t in zip(regions, transcripts_translate) if t]
