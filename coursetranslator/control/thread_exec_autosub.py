...
from coursetranslator.util.srtparser import SRTParser
from coursetranslator.util.util import MyUtil
from coursetranslator.control.ctr_autosub import Ctr_Autosub
import os
...
class Thread_Exec_Autosub(QThread):
    ...
    def __updateProgressFileYofN(self, currentIndex, countFiles ):
        self.signalProgressFileYofN.emit("Arhivo " + str(currentIndex+1) + " de " +str(countFiles))
    ...
    def __runAutosubForMedia(self, index, langCode, langCodeTranslate, ApiKey):
        ...
        fOutput = Ctr_Autosub.generate_subtitles(source_path = sourceFile,
                                    ...
                                    src_language_translate = langCodeTranslate, 
                                    ApiKey = ApiKey,
                                    listener_progress = self.listenerProgress)
        #if nothing was returned
        if not fOutput:
            self.signalErrorMsg.emit("Error! No se pudo procesar el archivo " + sourceFile + ".")
        elif fOutput != -1:
    ...
    def __loopSelectedFiles(self):
        ...
        langCodeTranslate = self.objParamAutosub.langCodeTranslate
        ApiKey = self.objParamAutosub.ApiKey
        ...
        if not os.path.isdir(pathOutputFolder):
            ...
            self.signalErrorMsg.emit("Error! Carpeta destino invalida.")
        else:
            ...
            for i in range(nFiles):
                ...
                if not Ctr_Autosub.is_operation_canceled():
                    ...
                    self.__runAutosubForMedia(i, langCode, langCodeTranslate, ApiKey)
            ...
            if Ctr_Autosub.is_operation_canceled():
                ...
                self.listenerProgress("Proceso Finalizado ", 100)
